...
function createJewelryItem(item, index) {
    ...
    itemElement.innerHTML = `
        ...
        <div class="jewelry-info">
            ...
            <button class="add-to-cart" data-id="${item.id}" type="button">Add to cart</button>
        </div>
    `;
    
    // Prevent add-to-cart clicks from opening the modal
    itemElement.addEventListener('click', (e) => {
        if (e.target.closest('.add-to-cart')) return;
        openModal(item);
    });

    // direct add-to-cart handler: stops propagation and updates cart immediately
    const addBtn = itemElement.querySelector('.add-to-cart');
    if (addBtn) {
...
// Helper: Add item to cart in localStorage and update UI, then notify others
    ...
    try {
        ...
        if (idx > -1) {
            ...
            cart.push({
                ...
                image: product.image || product.filePath || product.img || '',
                quantity: 1
            });
        }
        localStorage.setItem(KEY, JSON.stringify(cart));

        // Dispatch event so UI (in other script) can update
