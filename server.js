...
// Add Stripe (optional - require STRIPE_SECRET_KEY in env)
let stripe = null;
try {
    ...
    console.log('Stripe module not installed or failed to initialize. Install "stripe" to enable payments.');
}
...
app.post('/api/create-checkout-session', async (req, res) => {
    try {
        ...
        const line_items = items.map(item => {
            ...
            // Convert relative image path like "./images/xxx" -> absolute URL if possible
            let imageUrl;
            if (item.image) {
                const cleaned = item.image.replace(/^\.\//, '').replace(/^\//, '');
                imageUrl = `${req.protocol}://${req.get('host')}/${cleaned}`;
            }
            return {
                price_data: {
                    currency: 'inr',
                    product_data: {
                        name: String(item.title || 'Item').substring(0, 255),
        ...
        const session = await stripe.checkout.sessions.create({
            ...
            line_items,
            mode: 'payment',
            success_url: `${req.headers.origin || `http://localhost:${PORT}`}/?checkout=success`,
            cancel_url: `${req.headers.origin || `http://localhost:${PORT}`}/?checkout=cancel`
        });
